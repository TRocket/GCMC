...
void write_bytes(tcp::socket &sock, unsigned char *bytedata, int len) {
	for (int i = 0; i < len; ++i) {
		write_byte(sock, bytedata[i]);
	}
}

void write_short(tcp::socket &sock, short shortdata) {
	shortdata = __builtin_bswap32(shortdata << 16);
	unsigned char data[2];
	data[0] = shortdata;
	data[1] = shortdata >> 8;
	cout << "data: " << std::hex << (long) data << endl;
...
int main() {
	...
	for (;;) {
		...
		short protocol_ver;
		short str_len;
		unsigned char packetID = read_byte(socket);
		std::string username_str;
		std::string serveraddr_str;
		int port;
		unsigned char data[42] = { 0x00, 0xA7, 0x00, 0x31, 0x00, 0x00, 0x00,
				0x36, 0x00, 0x31, 0x00, 0x00, 0x00, 0x31, 0x00, 0x2E, 0x00,
				0x35, 0x00, 0x2E, 0x00, 0x32, 0x00, 0x00, 0x00, 0x47, 0x00,
				0x43, 0x00, 0x4D, 0x00, 0x43, 0x00, 0x00, 0x00, 0x30, 0x00,
				0x00, 0x00, 0x32, 0x00, 0x30 };
		cout << "packet ID " << std::hex << (int) packetID << endl;
		switch (packetID) {
		case 0x02:
			...
			write_byte(socket, 0xA);
			write_byte(socket, 0xB);
			write_byte(socket, 0xC);
			write_byte(socket, 0xD);
			write_byte(socket, 0xE);
			write_short(socket, 4);
			write_byte(socket, 0xF);
			write_byte(socket, 0x11);
			write_byte(socket, 0xF);
			write_byte(socket, 0x11);
		...
		case 0xFE:
			cout << read_byte(socket);
			write_byte(socket, 0xFF);
			write_short(socket, 21);
			write_bytes(socket, data, 42);
